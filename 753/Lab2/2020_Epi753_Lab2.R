# Known Issues
# - Multivariable log bionomial models that include smoking throw an error,
#       but univariable models with smoking do not.
# - I need to add robust variance estimation to Poisson model (Model 6).
...
# Note: is.na() returns TRUE/1 if a value is missing and FALSE/0 if a value is
# not missing. We use sum() to count the number of times TRUE/1 is returned.
...
# Means
t.test(age ~ death, data = dataset, var.equal = TRUE)

# Proportions
# Note: The mean of a binary variable coded 0/1 is the proportion equal to 1.
dataset %>%
    group_by(death) %>%
...
summary(glm(death ~ sex,       data = dataset, family = binomial(link = log)))
summary(glm(death ~ black,     data = dataset, family = binomial(link = log)))
summary(glm(death ~ idu,       data = dataset, family = binomial(link = log)))
summary(glm(death ~ hcv,       data = dataset, family = binomial(link = log)))
summary(glm(death ~ hbv,       data = dataset, family = binomial(link = log)))
summary(glm(death ~ smoking,   data = dataset, family = binomial(link = log)))
summary(glm(death ~ htn,       data = dataset, family = binomial(link = log)))
summary(glm(death ~ diabetes,  data = dataset, family = binomial(link = log)))
summary(glm(death ~ cd4gte350, data = dataset, family = binomial(link = log)))
summary(glm(death ~ ud_vl200,  data = dataset, family = binomial(link = log)))
summary(glm(death ~ art,       data = dataset, family = binomial(link = log)))
...
# Note: We can use coef() to return the coefficients, i.e., log relative risks.
coef(glm(death ~ age10, data = dataset, family = binomial(link = log)))

# Note: We can use coef() with exp() to return the exponentiated coefficients, 
# i.e., relative risks.
...
# Note: First, fit a model with all covariates (the starting model).
# start <- glm(death ~ age10 + black + idu + hbv + htn + diabetes + smoking + 
#    cd4gte350 + ud_vl200, data = dataset, family = binomial(link = "log"))
...
# Model 5: Model 1 + sex (demonstrating a model that will not converge)
glm(death ~ age10 + black + idu + hbv + smoking + htn + diabetes + cd4gte350 + 
    ud_vl200 + sex, data = dataset, family = binomial(link = "log"))

# Model 6: Model 5 + sex Using a Multivariable Poisson Regression Model
glm(death ~ age10 + black + idu + hbv + smoking + htn + diabetes + cd4gte350 + 
    ud_vl200 + sex, data = dataset, family = poisson(link = "log"))
